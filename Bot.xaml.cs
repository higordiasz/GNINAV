...
#endregion
#region Classes
using GNINAV.Controllers;
using GNINAV.Models;
using GNINAV.Models.Instagrams;
using GNINAV.GNI.Controllers;
using GNINAV.GNI.Models;
using GNINAV.Models.Blocks;
using Block = GNINAV.Models.Blocks.Block;
...
namespace GNINAV
{
    ...
    public partial class Bot : Window
    {
        ...
        private string TokenGNI { get; set; }
        ...
        #region GNI
        /// <summary>
        /// Realizar login no GNI através do Token
        /// </summary>
        /// <returns>Bool</returns>
        private async Task<bool> LoginGNI()
        {
            ...
            if (File.Exists($@"{dir}\Config\gni.txt"))
            {
                ...
                if (linhas.Length == 1)
                {
                    ...
                    if (File.Exists(@$"{dir}\Config\sessionid.txt"))
                    {
                        ...
                        {
                            //Realizar login
                            var ret = GniController.LoginGNI(TokenGNI);
                            if (ret.Status == "success")
                            {
                                SESSIONID = ret.SESSIONID;
                                UserID = ret.UserID;
                                string text = $"{TokenGNI};{SESSIONID};{UserID}";
                                File.WriteAllText(@$"{dir}\Config\sessionid.txt", text);
                                return true;
                            }
                            else
                            {
                                LogMessage($"Erro ao realizar login na plataforma, cheque o seu token");
                                LogMessage($"Retorno da API: {ret.Message}");
                                return false;
                            }
                        }
                    }
            ...
            {
                LogMessage($"Não foi possivel carregar o token do GNI");
                return false;
            }
        }
        /// <summary>
        /// Procura uma tarefa no Ganhar no Insta
        /// </summary>
        /// <param name="i">Index da conta</param>
        /// <returns>ReturnTask</returns>
        private async Task<ReturnTask> TarefaGNI(int i)
        {
            var task = GniController.GetTask(TokenGNI, InstagramGNI[i].ContaID, SESSIONID);
            if (task.Status != "ENCONTRADA")
            {
                if (Global.Trocar == false || Grupo.Contas.Count < 2)
                {
                    var j = 0;
                    bool taskCheck = false;
                    while (j < 25 && taskCheck == false)
                    {
                        task = GniController.GetTask(TokenGNI, InstagramGNI[i].ContaID, SESSIONID);
                        if (task.Status == "ENCONTRADA")
                        {
                            taskCheck = true;
                        }
            ...
            try
            {
                ...
                if (Directory.Exists($@"{dir}\logs"))
                {
                    ...
                    if (File.Exists($@"{dir}\logs\{data}.txt"))
                    {
                        ...
                        list.Add($"GNINAV {HorarioString()} {message}");
                        File.WriteAllLines($@"{dir}\logs\{data}.txt", list);
                        return;
                    }
                    else
                    {
                        string[] linhas = { $"GNINAV {HorarioString()} {message}" };
        ...
        {
            var dir = Directory.GetCurrentDirectory();
            var directory = $@"{dir}\logs\{name}.png";
            Screenshot ss = ((ITakesScreenshot)Driver).GetScreenshot();
            ss.SaveAsFile(directory,
        ...
        private async void InitializeDados()
        {
            ...
            {
                LogMessage($"O grupo '{GNINAV.App.GrupoName}' não possui contas para rodar o bot");
                System.Windows.Application.Current.Shutdown();
                return;
            }
        ...
        private async Task RodarConta()
        {
            ...
            if (i < 0)
            {
                var res = GniController.CheckAccount(TokenGNI, Contas[Index].Username, SESSIONID);
            ...
            try
            {
                await ConsoleMessage("Procurando tarefa", 1);
                var task = await TarefaGNI(i);
                if (task.Status == "ENCONTRADA")
                {
                    await ConsoleMessage($"Tarefa encontrada | Tipo: {task.Tipo} | Alvo: {task.URL}", 1);
                    ...
                    while (k < Global.Quantidade && sair == false)
                    {
                        ...
                        if (task.Status == "ENCONTRADA")
                        {
                            ...
                            if (res.Status != 1)
                            {
                                ...
                                var ret = GniController.ConfirmTask(TokenGNI, InstagramGNI[i].ContaID, SESSIONID, task.PedidoID, 2);
                                if (ret.Status == "success")
                                {
                                ...
                                else
                                {
                                    ...
                                    await ConsoleMessage(ret.Message, 1);
                                }
                            }
                            else
                            {
                                ...
                                var ret = GniController.ConfirmTask(TokenGNI, InstagramGNI[i].ContaID, SESSIONID, task.PedidoID, 1);
                                if (ret.Status == "interno")
                                {
                                    LogMessage($"Erro ao confirmar a tarefa: {ret.Message}");
                                    int q = 0;
                                    while (q < 3 && ret.Status == "interno")
                                    {
                                        ret = GniController.ConfirmTask(TokenGNI, InstagramGNI[i].ContaID, SESSIONID, task.PedidoID, 1);
                                        q++;
                                    }
                                }
                                if (ret.Status == "success")
                                ...
                                {
                                    await ConsoleMessage($"Não foi possivel confirmar a tarefa pelo seguinte motivo: {ret.Message}", 4);
                                }
                            }
                        }
                        else
                ...
                {
                    await ConsoleMessage("Tarefa não encontrada", 1);
                    await ConsoleMessage("Proteção contra multiplos logins, indo para proxima conta", 1);
                    if (Contas.Count > 1)
                        InstagramGNI[i].isLogged = false;
                    return;
                }
        ...
        private async Task<Retorno> RealizarTarefa(ReturnTask task, int index)
        {
            ...
            try
            {
                ...
                {
                    string alvo = "";
                    if (task.URL.IndexOf("instagram.com") > -1)
                    {
                        var array = task.URL.Split("/");
                        if (array[^1] == "")
                            alvo = array[^2];
                        else
                            alvo = array[^1];
                ...
                {
                    if (task.Tipo.ToLower() == "curtir")
                    {
                        string link = "";
                        if (task.URL.IndexOf("instagram.com") > -1)
                            link = task.URL;
                        else
                            link = $"https://www.instagram.com/p/{task.URL}/";
                    ...
                    {
                        ret.Status = -1;
                        ret.Response = $"O tipo de tarefa '{task.Tipo}' não está configurada";
                        return ret;
                    }
                }
            }
